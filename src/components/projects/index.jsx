...
const Project = () => {
  ...
  return (
    <div className={"w-full mx-auto "}>
      <div
        id="Project"
        className="max-w-[800px] mx-auto w-full h-full min-h-screen pt-24 pb-16 px-4 md:px-0 relative"
      >
        <h2
          className={` text-lg uppercase font-bold mb-10 underline underline-offset-8 ${
          ...
          {projectBtn.map((item) => {
            return (
              <button
                key={item.name}
                onClick={item.fun}
                className={`border px-4 py-1 rounded-xl drop-shadow-lg mb-4 ${
                  active === item.name.toLowerCase()
                    ? `${
                        theme === "light"
                          ? " border-blue-600 bg-blue-600 text-white"
                          : "border-[#FF0063] bg-[#FF0063] text-white"
                      }`
                    : ""
                }`}
              >
        ...
        <div className="flex items-center justify-center w-full ">
          <div className="flex flex-col items-center mt-16">
            <h1 className="text-2xl text-center py-4 text-black dark:text-[#CDD6F7]">
